...
export default function SearchBar({ onData, recipes, setRecipes  }) {
    ...
    const handleHeartClick = () => {
        const updatedRecipes = recipes.map(recipe => {
            if (recipe.heartClicked) {
                return recipe;
            }
            return {...recipe, heartClicked: true};
        });
        const filteredRecipes = updatedRecipes.filter(recipe => recipe.heartClicked);
        ...
        <>
            <div className={"mt-8 mb-4"}>
                <form className={"flex items-center gap-4"}>
                    ...
                    <div>
                        <button className={"border-2 rounded-md px-4 py-2"} onClick={handleHeartClick}>
